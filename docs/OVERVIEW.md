...
- **Pages (`apps/frontend/app/pages`):** Define the different routes and views of the application, now largely migrated to the `/apps/frontend/app` folder structure for a Next.js app router.
- **Components (`apps/frontend/components`):** Reusable UI components.
- **Database Client (`apps/frontend/db`):** Uses Drizzle ORM to interact with the PostgreSQL database. Includes a schema definition (`apps/frontend/db/schema.ts`) and utility functions.
- **Models (Typescript) (`apps/frontend/models`):** Define TypeScript types for data objects, mirroring some of the backend models.
- **Utilities (`apps/frontend/utils`):** Helper functions for formatting, assertions, OAuth, and other common tasks.
- **tRPC (`apps/frontend/trpc`):** Provides a type-safe API layer between the frontend and backend using tRPC. Routes are defined in `apps/frontend/trpc/routes`. Uses SuperJSON for serialization.
