  ...
  describe "#process" do
    describe "creating an invoice" do
      ...
      it "fails to create an invoice if the equity portion makes up less than one share" do
        ...
        expect(Bugsnag).to receive(:notify).with("InvoiceEquityCalculator: Calculated equity amount rounds to zero shares for CompanyWorker #{contractor.id}. Company needs to create proper equity grant.")
        expect do
          result = invoice_service.process
