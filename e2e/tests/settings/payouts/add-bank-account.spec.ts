...
test.describe("Bank account settings", () => {
  ...
  test.describe("Submit button behavior", () => {
    ...
    test("disables save button when validation errors exist", async ({ page }) => {
      await page.getByRole("link", { name: "Settings" }).click();
      await page.getByRole("link", { name: "Payouts" }).click();
      await page.getByRole("button", { name: "Add bank account" }).click();
      ...
      await page.getByLabel("Full name of the account holder").fill("A");
      await page.getByLabel("Routing number").fill("123");
      await page.getByLabel("Account number").fill("1");
      await page.getByRole("button", { name: "Continue" }).click();
      await page.getByLabel("Country").click();
      await page.getByRole("option", { name: "United States", exact: true }).click();
